...
## fit GAMLSS model for the healthy subsample of GSED.
# IE 23-5-2024
library(tidyr)
library(gseddata)

# get model object and extract D-scores - replace for onedrive d
model <- readRDS("C:/Users/eekhouti/OneDrive - TNO/Projecten/gsed/GSED model/293_0/model.rds")

## extract healthy ID's from onedrive
healthy_taz <- readxl::read_excel("C:/Users/eekhouti/World Health Organization/CAVALLERA, Vanessa - GSED Validation 2021_phase I/Pemba Validation/TZA Healthy SAMPLE IDS  19Nov2021.xlsx") %>%
  rename(GSED_ID = gsed_id) %>%
  mutate(COUNTRY = "Tanzania") %>%
  select(GSED_ID, HEALTHY, COUNTRY)

healthy_pak <- readxl::read_excel("C:/Users/eekhouti/World Health Organization/CAVALLERA, Vanessa - GSED Validation 2021_phase I/Pakistan Validation/PAKISTAN Healthy SAMPLE IDS 16Nov2021.xlsx", sheet = "List of IDs") %>%
  rename(GSED_ID = `GSED ID`) %>%
  mutate(COUNTRY = "Pakistan",
         HEALTHY = ifelse(is.na(`Healthy subsample`), 0, 1)) %>%
  select(GSED_ID, HEALTHY, COUNTRY)

healthy_ban <- readxl::read_excel("C:/Users/eekhouti/World Health Organization/CAVALLERA, Vanessa - GSED Validation 2021_phase I/Bangladesh Validation/BANGLADESH Healthy SAMPLE IDS 16Nov2021.xlsx", sheet = "List OF IDs") %>%
  mutate(HEALTHY = ifelse(Sample_Type == "Healthy", 1, 0),
         COUNTRY = "Bangladesh") %>%
  select(GSED_ID, HEALTHY, COUNTRY)

healthy <-  rbind(healthy_taz, healthy_pak, healthy_ban) %>%
  mutate(subjido = GSED_ID,
         cohort = strtrim(subjido, 7),
         cohort = recode(cohort, "11-GSED" = "GSED-BGD", "17-GSED" = "GSED-PAK", "20-GSED" = "GSED-TZA"),
         cohortn = as.integer(strtrim(subjido, 2)) + 100L,
         subjid = cohortn * 100000L + as.integer(substr(subjido, 9, 12)))
...
##compare with phase1 total
ref_phase1_healthy <- reference %>%
  mutate(age = year,
         pop = "phase1_healthy",
  ...
  pivot_longer(starts_with("SD"), names_to = "SD", values_to = "D")

ref_phase1 <- get_reference()%>%
  pivot_longer(starts_with("SD"), names_to = "SD", values_to = "D")


ggplot() +
  geom_line(data = ref_phase1, aes(age, D, group = SD, color = "full sample"))+
  geom_line(data = ref_phase1_healthy, aes(age, D, group = SD, color = "healthy sample"))
