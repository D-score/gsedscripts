...
if (packageVersion("gsedread") < "0.16.0") {
  stop("Needs gsedread 0.16.0")
}
if (packageVersion("dfine") < "0.13.0") {
  stop("Needs gsedread 0.13.0")
}
...
responses_lf <- responses |>
  ...
  mutate(pair = ifelse(is.na(pair), -1L, pair), ins = "lf") |>
  select(subjid, agedays, pair, ins, item, response)
...
counts <- sapply(data[items], function(x) {
  c(count_0 = sum(x == 0, na.rm = TRUE), count_1 = sum(x == 1, na.rm = TRUE))
})
...
dif_table <- left_join(
  dif_table,
  model$item_fit[, c("item", "infit", "outfit")],
  by = c("item" = "item")
)
dif_table <- left_join(
  dif_table,
  model$itembank[, c("item", "tau", "label")],
  by = c("item" = "item")
)
...
oldpar <- par(mfrow = c(2, 2))
hist(
  dif_table_sf$MH_stat,
  breaks = c(seq(0, 30, 0.5), Inf),
  xlim = c(0, 30),
  ylim = c(0, 0.45),
  main = "SF: Generalized Mantel-Haenszel",
  xlab = "MH DIF-statistic"
)
hist(
  dif_table_sf$LR_deltaR2,
  breaks = c(seq(0, 0.3, 0.005), Inf),
  xlim = c(0, 0.3),
  main = "SF: Generalized logistic regression",
  xlab = "Delta R2 Statistic"
)
hist(
  dif_table_lf$MH_stat,
  breaks = c(seq(0, 30, 0.5), Inf),
  xlim = c(0, 30),
  ylim = c(0, 0.45),
  main = "LF: Generalized Mantel-Haenszel",
  xlab = "MH DIF-statistic"
)
hist(
  dif_table_lf$LR_deltaR2,
  breaks = c(seq(0, 0.3, 0.005), Inf),
  xlim = c(0, 0.3),
  main = "LF: Generalized logistic regression",
  xlab = "Delta R2 Statistic"
)
par <- par(mfrow = c(1, 1))
...
write.csv(
  dif_table_sf,
  file = file.path(path, "dif_table_sf.csv"),
  row.names = FALSE
)
write.csv(
  dif_table_lf,
  file = file.path(path, "dif_table_lf.csv"),
  row.names = FALSE
)
...
hist(
  q3$q3.long$Q3[as.numeric(q3$q3.long$N) > 200],
  freq = TRUE,
  breaks = c(-Inf, seq(-0.5, 0.5, 0.01), Inf),
  xlim = c(-0.5, 0.5),
  main = "Q3 correlations",
  xlab = "Correlation"
)
...
filled.contour(
  ...
  1:p,
  obs,
  col = colorRampPalette(c("blue", "white", "red"))(17),
  xlab = "Item",
  ylab = "Item"
)
filled.contour(
