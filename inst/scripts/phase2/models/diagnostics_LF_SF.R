...
path_new <- file.path(Sys.getenv("GSED_PHASE2"), "202510", model_name)
...
plot1 <- ggplot(
  joined,
  aes(x = d_lf, y = d_sf, group = cohort, colour = cohort)
) +
...
plot2 <- ggplot(
  joined,
  aes(x = d_mean, y = d_diff, group = cohort, colour = cohort)
) +
  scale_colour_manual(values = col_manual, na.value = "grey") +
  scale_x_continuous("(LF + SF) / 2", limits = c(10, 80)) +
  scale_y_continuous(
    paste0("LF - SF"),
    breaks = seq(-10, 10, 2),
    limits = c(-10, +10)
...
plot3 <- ggplot(
  joined,
  aes(x = a, y = daz_lf, group = cohort, colour = cohort)
) +
  scale_colour_manual(values = col_manual, na.value = "grey") +
...
plot4 <- ggplot(
  joined,
  aes(x = a, y = daz_sf, group = cohort, colour = cohort)
) +
  scale_colour_manual(values = col_manual, na.value = "grey") +
  scale_x_continuous("Age (years)", limits = c(0, 3.5)) +
...
plot5 <- ggplot(
  joined,
  aes(x = daz_lf, y = daz_sf, group = cohort, colour = cohort)
) +
  scale_colour_manual(values = col_manual, na.value = "grey") +
...
plot6 <- ggplot(
  joined,
  aes(x = daz_mean, y = daz_diff, group = cohort, colour = cohort)
) +
  scale_colour_manual(values = col_manual, na.value = "grey") +
  scale_x_continuous(
    "(LF + SF) / 2 (DAZ preliminary_standards)",
    limits = c(-4, 4)
  ) +
  scale_y_continuous(
    "LF - SF (DAZ preliminary_standards)",
    limits = c(-4, +4)
  ) +
  coord_fixed() +
  geom_hline(yintercept = c(0), colour = "grey", linewidth = 1.5) +
...
dec$ins <- ifelse(dec$instrument == "gs1", "SF", "LF")
