...
data <- gsedscripts::merge_sf_lf_bsid(work)
...
has_sf <- apply(!is.na(select(data, any_of(items_sf))), 1L, any)
has_lf <- apply(!is.na(select(data, any_of(items_lf))), 1L, any)
has_bsid <- apply(!is.na(select(data, any_of(items_bsid))), 1L, any)
...
remove_by3 <- setdiff(items_bsid, keep_by3)
