...
# Updated 20240703, update to dscore 1.9.0
...
plot3 <- ggplot(joined, aes(x = a, y = daz_lf, group = cohort, colour = cohort)) +
  ...
  scale_y_continuous("LF (DAZ)", limits = c(-5, 5)) +
...
plot4 <- ggplot(joined, aes(x = a, y = daz_sf, group = cohort, colour = cohort)) +
  scale_colour_manual(values = col_manual, na.value = "grey") +
  scale_x_continuous("Age (years)", limits = c(0, 3.5)) +
  scale_y_continuous("SF (DAZ)", limits = c(-5, 5)) +
...
plot5 <- ggplot(joined, aes(x = daz_lf, y = daz_sf, group = cohort, colour = cohort)) +
  scale_colour_manual(values = col_manual, na.value = "grey") +
  scale_x_continuous("LF (DAZ)", limits = c(-5, 5)) +
  scale_y_continuous("SF (DAZ)", limits = c(-5, 5)) +
  coord_fixed() +
  geom_abline(slope = 1, colour = "grey", linewidth = 1.5) +
  geom_point(
...
plot6 <- ggplot(joined, aes(x = daz_mean, y = daz_diff, group = cohort, colour = cohort)) +
  scale_colour_manual(values = col_manual, na.value = "grey") +
  scale_x_continuous("(LF + SF) / 2 (DAZ)", limits = c(-4, 4)) +
  scale_y_continuous("LF - SF (DAZ)", limits = c(-4, +4)) +
  coord_fixed() +
  geom_hline(yintercept = c(0), colour = "grey", size = 1.5) +
  geom_smooth(se = TRUE, colour = "grey60", size = 1.5) +
  geom_point(
...
# compare difficulty estimates with key gsed1912
items_gsed <- gsedread::rename_vector(c(items_lf, items_sf), lexin = "gsed2", lexout = "gsed")
tau_gsed1912 <- get_tau(items = items_gsed, key = "gsed1912")
...
g <- g1 +
  geom_text(aes(y = tau_gsed1912, x = 1, label = item), data = df[df$instrument == "LF", ],
