...
The required input consists of *item level* responses on milestones collected using instruments for measuring child development, including the GSED LF.
...
----------:|:-------------------- |:-------------
1-3        | instrument           | `gl1`
4-5        | developmental domain | `gm`
6          | administration mode  | `d`
7-9        | item number          | `018`

Thus, item `gl1gmd018` refers to the 18th item in GSED LF and measures motor development through direct observation. The label of the item can be obtained by 

```{r getlabels}
library(dscore)
get_labels("gl1gmd018")
```
...
The GSED LF V1 contains 155 items and has instrument code `gl1`. Obtain the full list of item names for GSED LF as 
...
It is possible to calculate $D$-score for item subsets by setting the `items` argument. We do not advertise this option for practical application, but suppose we are interested in the $D$-score based on items from `gl1` for domains `gm` (motor) only. The "motor" $D$-score can be calculated as follows:

```{r}
items_motor <- get_itemnames(instrument = "gl1", domain = "gm")
...
The script below creates a figure with -2SD, 0SD and +2SD centiles plus 10 $D$-scores (10 LF) for the `lf2` data.
