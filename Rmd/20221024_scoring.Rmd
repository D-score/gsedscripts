...
Suppose that you have administered SF, LF or both to one or more children. A next step is to calculate the developmental score ($D$-score) and the age-adjested equivalent (DAZ) for each child. This step is known as **scoring**. The present section provides recipes on how to calculate these score. We may pick one of the following two methods:
...
2. `R` package `dscore`. The `R` package [dscore]<https://CRAN.R-project.org/package=dscore> is a flexible option with all the tools needed to calculate the $D$-score. It is an excellent choice for users familiar with `R` and users who like to incorporate $D$-score calculations into a workflow.
...
Note that the second route requires that you have a local C++ compiler available for building the package from source. We use here `dscore 1.6.5`.

```{r include=FALSE}
stopifnot(packageVersion("dscore") >= "1.6.5")
```
